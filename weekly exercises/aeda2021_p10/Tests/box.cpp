...
// TODO e)
string Box::printContent() {
    string s= "Box " + to_string(id);
    if(free==capacity) return s+" empty!\n";
    s += " [ ";
    stack<Object> objAux = objects;
    while(!objAux.empty()){
        s += "O" + to_string(objAux.top().getID()) + ":" + to_string(objAux.top().getWeight()) + " ";
        objAux.pop();
    }
    s += "]";
	return s;
